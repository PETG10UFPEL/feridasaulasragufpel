...
with st.sidebar:
    ...
    if is_admin:
        ...
        if st.button("1) Sincronizar arquivos do Drive"):
            ...
            if files:
                st.success(f"Baixados {len(files)} arquivos novos para data/raw_docs")
            else:
...
def cached_answer(q, patient, k):
    ...
    # O cache é limpo automaticamente ao recriar o índice (ver botão "Recriar índice" na sidebar)
